...
class SwinTransformer3DModel(nn.Module):
    def __init__(self, num_classes, **kwargs):
        super(SwinTransformer3DModel, self).__init__()
        self.backbone = SwinTransformer3D(**kwargs)
        self.avgpool = nn.AdaptiveAvgPool3d(1)
        self.head = nn.Linear(
            self.backbone.num_features, num_classes) if num_classes > 0 else nn.Identity()
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        for line in lines:
            ...
            data.append((video_path, int(label)))
    ...
    def __getitem__(self, idx):
        ...
        # torchvision.transforms._transforms_video.CenterCropVideo(crop_size=(224,224)),  # 调整图像大小
        # torchvision.transforms.ToTensor(),  # 转换为Tensor
        # transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # 标准化
        ])
        if self.transform:
            frames = self.transform(frames)
        return frames.float()/ 255.0, label


if __name__ == "__main__":
    # model = swin_tiny_patch4_window7_224(num_classes=20,pretrained='modal_fusion\model\swin_tiny_patch244_window877_kinetics400_1k.pth')
    # print(model)
    # # model.init_weights()
    ...
    test_dataset = CustomDataset(file_path='kinetics400_tiny\kinetics_tiny_train_video.txt', root_dir='kinetics400_tiny/train', transform=None)
